...
func copyRetryable(old superAgentRetryable) superAgentRetryable {
	...
	newRetryable.NonRetryableStatus = make([]int, len(old.NonRetryableStatus))
	for i := range old.NonRetryableStatus {
		newRetryable.NonRetryableStatus[i] = old.NonRetryableStatus[i]
	}
	return newRetryable
}
...
func (s *SuperAgent) RetryRev(retryerCount int, retryerTime time.Duration, statusCodeRev ...int) *SuperAgent {
	for _, code := range statusCodeRev {
		statusText := http.StatusText(code)
		if len(statusText) == 0 {
			s.Errors = append(s.Errors, errors.New("StatusCode '"+strconv.Itoa(code)+"' doesn't exist in http package"))
		}
	...
	s.Retryable = superAgentRetryable{
		NonRetryableStatus: statusCodeRev,
		RetryerTime:        retryerTime,
		RetryerCount:       retryerCount,
		Attempt:            0,
		Enable:             true,
...
func (s *SuperAgent) isRetryableRequest(resp Response) bool {
	if s.Retryable.Enable && s.Retryable.Attempt < s.Retryable.RetryerCount &&
		(contains(resp.StatusCode, s.Retryable.RetryableStatus) ||
			!contains(resp.StatusCode, s.Retryable.NonRetryableStatus)) {

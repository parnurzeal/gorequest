...
func TestMultipartRequest(t *testing.T) {
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case case16_send_file_by_content_with_name_and_with_fieldname,
			...
			checkFile(t, r.MultipartForm.File["my_fieldname"][0])
		case case161_send_file_by_content_with_name_and_fieldname_and_mimetype,
			case121_send_file_by_path_with_name_and_fieldname_and_mimetype:
			if len(r.MultipartForm.File) != 1 {
				t.Error("Expected length of files:[] == 1", "| but got", len(r.MultipartForm.File))
			}
			if _, ok := r.MultipartForm.File["my_fieldname"]; !ok {
...
func TestEndStruct(t *testing.T) {
	...
	// Callback.
	{
		resp, bodyBytes, errs := New().Get(ts.URL).
			EndStruct(func(resp Response, v interface{}, body []byte, errs []error) {
				if len(errs) > 0 {
					t.Fatalf("Unexpected errors: %s", errs)
				}
				if resp.StatusCode != 200 {
